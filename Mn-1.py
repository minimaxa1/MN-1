...
THEMES = {
    ...
    "grey": {
        ...
        "button_fg": COLOR_MID_GRAY,    # <<< CHANGED: Button bg matches frame
        "button_text": COLOR_WHITE,
    ...
    "red": {
        ...
        "label_fg": COLOR_WHITE,
        "button_fg": COLOR_DEEP_RED,    # <<< CHANGED: Button bg matches frame
        "button_text": COLOR_WHITE,
...
class MN1MusicPlayer:
    ...
    def create_controls(self):
        ...
        # Define style for the theme toggle button (needs to be wide enough for NM-1)
        theme_button_kwargs = {"font": self.normal_font, "border_width": 0, "corner_radius": 0, "width": 65, "height": 25} # <<< Changed width
        ...
        self.theme_toggle_button = ctk.CTkButton(
            self.extra_frame,
            text="NM-1", # Initial text set here, updated in apply_theme
    ...
    def apply_theme(self):
        ...
        # <<< CHANGED: Theme button text is now always "NM-1"
        theme_button_display_text = "NM-1"
        ...
        try:
            ...
            if self.theme_toggle_button and self.theme_toggle_button.winfo_exists():
                self.theme_toggle_button.configure(
                    text=theme_button_display_text, # <<< Set text to "NM-1"
                )
    ...
    # --- ... (play_music, pause, stop, previous_song, next_song) ---
    # --- ... (volume_adjust, toggle_mute, toggle_mix, toggle_loop) ---
    # --- ... (slider/waveform interaction methods) ---
    # --- ... (seek_song, update_song_info) ---
    # --- ... (waveform generation methods) ---
    # --- ... (plot drawing methods) ---
    # --- ... (time update thread methods) ---
    # --- ... (song end handling methods) ---
    # --- ... (play_random_song, abort_waveform_generation, on_closing) ---

    # --- [No changes needed below this line for this request] ---
    ...
    def slide_song(self, value):
        # Only update display if actively dragging slider
        if not self.slider_active: return
        position = float(value)
        self.song_time = position # Keep internal time synced while sliding
        ...
        # Optionally update oscilloscope while sliding (can be jerky)
    ...
    def _update_position_from_waveform_event(self, event):
        ...
        # Optionally update oscilloscope while dragging (can be jerky)
        # if self.raw_sample_data is not None: self.update_oscilloscope()
